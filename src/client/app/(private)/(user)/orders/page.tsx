...
const OrderCard = ({ order }: { order: any }) => {
  const currentStatus =
    order?.tracking?.currentStatus || order?.transaction?.status || order?.status;
...
const UserOrders = () => {
  ...
  const { data, isLoading, error, refetch } = useGetUserOrdersQuery(
    {},
    {
      pollingInterval: fromCheckout ? 3000 : 0,
          ...
          <div className="text-center py-12">
            ...
            {fromCheckout && (
              <p className="mt-2 text-sm text-gray-500">
                Your recent payment was successful. We&apos;re still waiting for
                the order to appear, so this page is retrying automatically.
              </p>
            )}
            <Link
              href="/products"
