...
import { useCreateOrderSupportHandoffMutation } from "@/app/store/apis/OrderApi";

const SupportPage = () => {
  const searchParams = useSearchParams();
  const preselectedChatId = searchParams.get("chatId");
  const handoffOrderId = searchParams.get("orderId");
  const handoffGoalStatus = searchParams.get("goalStatus");
  const handoffGoalReason = searchParams.get("goalReason");
  const handoffGoalTemplateId = searchParams.get("goalTemplateId");
  const [activeChatId, setActiveChatId] = useState<string | null>(null);
  const lastHandoffKeyRef = useRef<string | null>(null);
  console.log("activeChatId => ", activeChatId);
  const { data: chats, isLoading, refetch } = useGetUserChatsQuery(undefined);
  console.log("user chats => ", chats);

  const [createChat, { isLoading: isCreatingChat }] = useCreateChatMutation();
  const [createOrderSupportHandoff, { isLoading: isPreparingOrderSupport }] =
    useCreateOrderSupportHandoffMutation();
  const handoffKey = [
    handoffOrderId || "",
    handoffGoalStatus || "",
    handoffGoalReason || "",
    handoffGoalTemplateId || "",
  ].join(":");

  useEffect(() => {
    if (preselectedChatId && !activeChatId) {
      setActiveChatId(preselectedChatId);
    }
  }, [activeChatId, preselectedChatId]);

  useEffect(() => {
    if (preselectedChatId || !handoffOrderId) {
      return;
    }

    if (lastHandoffKeyRef.current === handoffKey) {
      return;
    }
    ...
    createOrderSupportHandoff({
      orderId: handoffOrderId,
      status: handoffGoalStatus || undefined,
      primaryReason: handoffGoalReason || undefined,
      goalTemplateId: handoffGoalTemplateId || undefined,
    })
      .unwrap()
      .then((result) => {
      ...
      .catch((error) => {
        console.error("Failed to prepare order support handoff:", error);
    ...
    <MainLayout>
      ...
      <div className="flex items-start justify-between p-8">
        ...
        <div className="flex-1">
          ...
          ) : isPreparingOrderSupport ? (
            <div className="flex items-center justify-center h-full text-gray-500">
              Preparing order support...
            </div>
          ) : (
            <div className="flex items-center justify-center h-full text-gray-500">
