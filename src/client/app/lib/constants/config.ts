...
const stripApiPath = (url: string) => url.replace(/\/api\/v1\/?$/, "");
...
export const SOCKET_URL =
  process.env.NODE_ENV === "production"
    ? process.env.NEXT_PUBLIC_SOCKET_URL_PROD ||
      stripApiPath(process.env.NEXT_PUBLIC_API_URL_PROD || PROD_API_URL)
    : process.env.NEXT_PUBLIC_SOCKET_URL_DEV ||
      stripApiPath(process.env.NEXT_PUBLIC_API_URL_DEV || DEV_API_URL);
