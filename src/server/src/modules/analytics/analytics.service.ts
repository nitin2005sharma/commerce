...
export class AnalyticsService {
  ...
  async getUserAnalytics(query: DateRangeQuery): Promise<UserAnalytics> {
    ...
    // Top users
const userOrdersMap = new Map<string, TopUser>();

for (const order of (orders as any[])) {
  const userId = String(order.userId);
  let existing = userOrdersMap.get(userId);
  ...
  existing.engagementScore += (interactions as any[]).filter(
...
const topUsers: TopUser[] = Array.from(userOrdersMap.values())
  .sort((a: TopUser, b: TopUser) => b.totalSpent - a.totalSpent)
  .slice(0, 5);
