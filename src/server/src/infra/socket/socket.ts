...
export class SocketManager {
  ...
  this.io = new SocketIOServer(httpServer, {
    ...
    this.io.on("connection", (socket: Socket) => {
      ...
      socket.on("joinUserOrders", (userId: string) => {
        socket.join(`user:${userId}:orders`);
        console.log(`Client ${socket.id} joined user:${userId}:orders`);
      });

      socket.on("joinSharedCart", (code: string) => {
        socket.join(`shared-cart:${code}`);
        console.log(`Client ${socket.id} joined shared-cart:${code}`);
      });

      socket.on(
        "sharedCartTyping",
        ({ code, user }: { code: string; user: string }) => {
          socket.to(`shared-cart:${code}`).emit("sharedCartTyping", {
            user,
          });
        }
      );
      ...
      socket.on(
        "sharedCartStopTyping",
        ({ code }: { code: string }) => {
          socket.to(`shared-cart:${code}`).emit("sharedCartStopTyping");
        }
      );
      ...
      socket.on("callAnswer", ({ chatId, answer, to }) => {
        socket.to(to).emit("callAnswer", { answer, from: socket.id });
      ...
      socket.on("iceCandidate", ({ chatId, candidate, to }) => {
        ...
        if (to) {
          ...
          socket
            .to(`chat:${chatId}`)
            .emit("iceCandidate", { candidate, from: socket.id });
        }
